...
public class AppExceptionMapper implements ExceptionMapper<Exception> {
	...
	if (e instanceof SqlException || e.getCause() != null) {
	    ...
	    // logger.info("SqlException - cause: " + clzName + " - message: " +
	    // e.getCause().getMessage());
	    details.setStatus(Status.BAD_REQUEST); // default value
	    if ("com.mysql.jdbc.exceptions.jdbc4.MySQLIntegrityConstraintViolationException".equals(clzName)) {
		details.setCode(20).setMessage("Constraint violation occured")
			.setDescription(e.getCause().getMessage());
	    } else if ("javax.persistence.PersistenceException".equals(clzName)) {
		details.setCode(20).setMessage("Persistence exception occured").setDescription(e.getMessage());
	    } else if ("org.hibernate.PersistentObjectException".equals(clzName)) {
		details.setCode(20).setMessage("Setting Primary key not allowed")
			.setDescription("Primary key Id can't be set explicitly");
	    } else if ("javax.persistence.NoResultException".equals(clzName)) {
		details.setCode(30).setMessage("Not Found").setDescription(e.getMessage()).setStatus(Status.NOT_FOUND);
	    } else if ("javax.persistence.QueryTimeoutException".equals(clzName)) {
		details.setCode(40).setMessage("Timeout").setDescription(e.getMessage())
			.setStatus(Status.INTERNAL_SERVER_ERROR);
	    } // defaults if no match is found above
	    else {
		details.setCode(1).setMessage("Internal Server Error").setDescription(e.getMessage())
			.setStatus(Status.INTERNAL_SERVER_ERROR);
	    }
	} else {
